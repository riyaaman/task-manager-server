...
function TodoList() {
  // Get UI State (filter/sort)
  const { state } = useTasks();

  // Get Server State (tasks/loading/error)
  const { tasks, isLoading, isError } = useTaskQuery();

  // Get the tasks filtered and sorted based on state
  const visibleTasks = getVisibleTasks(tasks, state.filter, state.sortBy);
  ...
  if (isError)
    return (
      <p className="error-message">
        ⚠️ Error fetching tasks. Please try again.
      </p>
    );
  return (
    <div className="todo-list-display">
